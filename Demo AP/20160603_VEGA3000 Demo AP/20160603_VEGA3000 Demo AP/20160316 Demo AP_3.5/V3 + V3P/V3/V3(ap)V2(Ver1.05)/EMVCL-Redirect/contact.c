...
#include "pciutility.h"
#include "../GetPin/pcipad.h"
...
extern USHORT CTOS_FunRedirectGetDevice(USHORT* usDeviceID, ULONG* ulPara);
extern USHORT CTOS_FunRedirectSetPort(BYTE bPort);
extern USHORT CTOS_FunRedirectGetPort(BYTE* bPort);
// --------------

extern BYTE g_bCOM;
extern USHORT g_usDeviceID;
extern ULONG g_ulPara;
extern BYTE g_bPort;

//--------------- PCIUtility (GetPin)
extern USHORT RXData(BYTE *data, USHORT *len);
extern USHORT TXData(BYTE *data, USHORT len);
extern USHORT DECData(IN BYTE KeyType, IN BYTE *pData, IN USHORT Len, OUT BYTE *pResult, OUT USHORT *pResultLen);
extern USHORT ENCData(IN BYTE KeyType, IN BYTE *pData, IN USHORT Len, OUT BYTE *pResult, OUT USHORT *pResultLen);

extern ULONG EMVCL_CancelTransaction(void);
...
void Tx(BYTE Type) {
    ...
    CTOS_FunRedirectSetDevice(d_DEVICE_ID_MAIN_DEVICE, 0x00);
...
//        CTOS_LCDTPrintXY(1, 1, "\fr    Approval    \fn");
        ...
        CTOS_FunRedirectSetDevice(d_DEVICE_ID_EXTERNAL_DEVICE, 0x00);
...
//            CTOS_LCDTPrintXY(1, 1, "\fr    Approval    \fn");
    ...
    if (Type == MSR) {
        PrintCenter(6, " Press Any Key ");
        CTOS_KBDGet(&key1);
//        CTOS_LEDSet(0, 0);
//        CTOS_LEDSet(2, 0);
    } else {
        CTOS_LCDTPrintXY(1, 6, "    Remove Card    ");
        CTOS_FunRedirectSetDevice(d_DEVICE_ID_EXTERNAL_DEVICE, 0x00);
        CTOS_LCDTPrintXY(1, 3, "                                ");
        CTOS_LCDTPrintXY(1, 4, "                                ");
        CTOS_LCDTPrintXY(1, 6, "    Remove Card    "); //Remove Card
        CTOS_Delay(1000);
...
USHORT InputPIN(void){
	...
	CTOS_LCDGShowBMPPic(0, 60, "fs_data/EnterPIN_V3P_420.bmp");
...
void Status(ULONG ulCurrentSe, ULONG ulTotalSz)
{
    CTOS_FunRedirectSetDevice(0, 0); 
    BYTE buff[48]={0};
    sprintf(buff,"%ld/%ld    ",ulCurrentSe,ulTotalSz);
    CTOS_LCDTPrintXY(1,11,"Receive Data");
    CTOS_LCDTPrintXY(5,12,buff);
}

BOOL TouchSignature(void){
	BYTE key;
	PCIUTILITY_EVENT stComm;
	USHORT usRet;
	
Start_Sign:
	CTOS_LCDGShowBMPPic(0, 60, "fs_data/WaitforSign_V3P_420.bmp");
	EMVCL_SetLED(0x0F, 0x08);
...
Continue_Sign:	
    ...
    PCIUtility_CommunicationInitialize(&stComm);
	
	CTOS_LCDGShowBMPPic(0, 60, "fs_data/Processing_V3P_420.bmp");
	usRet = PCIUtility_GetFile(MonoSignBMPFile, MonoSignBMPFile, Status);
...
//    CTOS_LCDTPrintXY(1, 3, " Please Insert or");
//    CTOS_LCDTPrintXY(1, 4, " Swipe card on Pinpan..");
    CTOS_LCDTPrintXY(1, 4, "      Waiting....   ");
    CTOS_LCDTPrintXY(1, 10, " \frX\fn:Exit");
    
    CTOS_FunRedirectSetDevice(d_DEVICE_ID_EXTERNAL_DEVICE, 0x00);
    //CTOS_LCDTClearDisplay();
//    CTOS_LCDBackGndColor(0x009CE4F5);
//    CTOS_LCDGShowBMPPic(0, 0, "background00.bmp");
    CTOS_LCDTPrintXY(1, 6, "                          ");
    CTOS_LCDTPrintXY(1, 3, " Please Insert or");
    CTOS_LCDTPrintXY(1, 4, " Swipe card..    ");
//    CTOS_LCDTPrintXY(1, 10, " \frX\fn:Exit");
    //CTOS_LCDBackGndColor(0x009CE4F5);
}
...
//    CTOS_LCDSelectMode(d_LCD_TEXT_320x240_MODE);
	...
	CTOS_LCDTPrintXY(1, 3, "Please Input Amount :     ");
	CTOS_LCDTPrintXY(1, 4, "                          ");
	//Input Amount
	if (AP_Get_Input(3, 4, 1, 12, NULL, sInputAmount, &c) != TRUE) {
		CTOS_LCDTClearDisplay();
