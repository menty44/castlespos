...
void Print_Receipt2(EMVCL_RC_DATA_EX *data, ULONG ulValue){
	...
	//sprintf(TransHis.ExpDate, "EXP. DATE: 20%s/%s", baTmp1, baTemp);
    CTOS_PrinterPutString(baBuf);
    CTOS_PrinterFline(5);
	...
	//CTOS_LanguagePrinterFontSize(d_FONT_8x16, d_FONT_PRINTER_USE, d_FONT_NO_SET_TYPE);
	int i, j;
	i = (int)ulValue / 100;
	j = (int)ulValue % 100;
	if(i == 0){
	...
	//CTOS_PrinterFontSelectMode(d_FONT_FNT_MODE);
	//CTOS_PrinterTTFSelect("ca_default.ttf", 1);

    stFONT_ATTRIB.FontSize = d_FONT_12x24;//d_FONT_8x16
    stFONT_ATTRIB.X_Zoom = 3;
    stFONT_ATTRIB.Y_Zoom = 2;//4
    memset(baTemp, 0x00, sizeof (baTemp));
