        ...
        switch (bKey) {
            ...
            case d_KBD_CLEAR: //Clear
                if (tempLen == 0) {
					...
					//CTOS_LCDTPrintXY((19 - tempLen), 6, strAmt);
					...
					if(tempLen < 3){
						if(Dollar_Sign == 1)
							...
							CTOS_LCDTPrintXY((18 - tempLen), 6, strAmt);
						//CTOS_LCDTPrintXY((19 - tempLen), 6, strAmt);
					}else{
						bAmt_Dec[0] = strAmt[tempLen-2];
						bAmt_Dec[1] = strAmt[tempLen-1];
						//CTOS_LCDTPrintXY((19 - tempLen - 1), 6, strAmt);
						...
						if(Dollar_Sign == 1){
							...
							CTOS_LCDTPrintXY((18 - tempLen - 1), 6, strAmt);
							CTOS_LCDTPrintXY(15, 6, ".");
							CTOS_LCDTPrintXY(16, 6, bAmt_Dec);
						}
					}
                }
                break;
...
//------------------------------------------------------------------------------
extern BYTE DebugLog[4096];
extern int DebugLen;

void AddHEX(BYTE *title, BYTE *hex, USHORT len)
{
...
//    CTOS_LCDGShowPic(95,55,(BYTE *)baLCDLogo,sizeof(baLCDLogo),142);     \\for 2.4" LCD
	...
	AddHEX("In Data", TransaRelatedData, stACTData.usTransactionDataLen);
...
void Transaction(void) {
    ...
    while (1) {
		...
		CTOS_LCDGShowBMPPic(0, 0, "fs_data/Welcome_V3P.bmp");//Device on V3P, so the bmp we used also in V3P(PCIPas_App's ap file)

		CTOS_FunRedirectSetDevice(0x00, 0x00); 

        ulTxntype = 0 + g_bTxntype;
		
		CTOS_LCDTPrintXY(1, 5, " Enter Amount: ");
		//CTOS_LCDTPrintXY(1, 6, "   $          0.00  ");
		if(Dollar_Sign == 1)
			CTOS_LCDTPrintXY(1, 6, "   $          0.00  ");
		else if(Dollar_Sign == 2)
		...
		CTOS_LCDGShowBMPPic(0, 0, "fs_data/SwipeInsertTap_V3P.bmp");
