...
USHORT InputPIN(void){
	...
	CTOS_LCDGShowBMPPic(0, 0, "fs_data/EnterPIN.bmp");
		...
		switch(ICCKey){
			case d_KBD_ENTER:
				if((ICCPINLen < 4) || (ICCPINLen > 12)){
					...
					if(strcmp(ICCPIN, "8418") == 0){
						...
						CTOS_LCDTPrintXY(1, 6, "     Wrong PIN!    ");
						CTOS_Delay(100);
						CTOS_Beep();
						CTOS_Delay(100);
						CTOS_Beep();
			...
			case d_KBD_CLEAR:
				if (ICCPINLen == 0) {
					...
					//CTOS_LCDTPrintXY((13 - ICCPINLen), 5, ICCStar);
					//CTOS_LCDTPrintXY((13 - ICCPINLen), 6, ICCPIN);
					if((ICCPINLen % 2) == 1)
						CTOS_LCDTPrintXY((11 - (ICCPINLen/2)), 5, ICCStar);
			...
			default:
				if((ICCKey >= '0' && ICCKey <= '9') && ICCPINLen < 12){
					...
					//CTOS_LCDTPrintXY((13 - ICCPINLen), 6, ICCPIN);
					if((ICCPINLen % 2) == 1)
						CTOS_LCDTPrintXY((10 - (ICCPINLen/2)), 5, ICCStar);
					else if((ICCPINLen % 2) == 0)
						CTOS_LCDTPrintXY((11 - (ICCPINLen/2)), 5, ICCStar);
...
USHORT Tx_EMV(void){
	...
	if(SC_Status & d_MK_SC_PRESENT)
	{
		...
		//CTOS_LCDTClearDisplay();
		CTOS_LCDGShowBMPPic(0, 0, "fs_data/Processing_V3P.bmp");
		...
		if(ICCTrade_Finishflag)
		{
			...
			CTOS_FunRedirectSetDevice(0x01, 0x00);
			CTOS_LCDGShowBMPPic(0, 0, "fs_data/Printing_V3P.bmp");
			prt_sample2(g_baInputAmt, 1);//sInputAmount
	...
	if(rtn != d_OK ||  usTk2Len == 0){
		...
		if(MSRTrade_Finishflag)
		{
			...
			CTOS_FunRedirectSetDevice(0x01, 0x00);
			CTOS_LCDGShowBMPPic(0, 0, "fs_data/Printing_V3P.bmp");
			prt_sample2(g_baInputAmt, 2);//sInputAmount
			return d_OK;
		}else
			//CTOS_LCDGShowBMPPic(0, 0, "background00.bmp");
			;
