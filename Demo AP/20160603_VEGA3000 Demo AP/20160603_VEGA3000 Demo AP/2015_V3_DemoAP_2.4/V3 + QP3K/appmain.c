        ...
        switch(bKey){
            ...
            case d_KBD_CLEAR: //Clear
                if (iLen == 0) {
					...
					//CTOS_LCDTPrintXY((19 - iLen), 6, baInput);
					
					CTOS_LCDTPrintXY(1, 6, "   $          0.00  ");
					if(iLen < 3){
						CTOS_LCDTPrintXY((19 - iLen), 6, baInput);
					}else{
						bAmt_Dec[0] = baInput[iLen-2];
						bAmt_Dec[1] = baInput[iLen-1];
						CTOS_LCDTPrintXY((19 - iLen - 1), 6, baInput);
						CTOS_LCDTPrintXY(16, 6, ".");
						CTOS_LCDTPrintXY(17, 6, bAmt_Dec);
					}
                }
                break;
            default:
...
//   sprintf(debugbuf, "0x%08X\n", ulAPRtn);
//   CTOS_LCDTPrintXY(1, 5, debugbuf);
    if (ulAPRtn == d_NO_ERROR) {
...
//    sprintf(debugbuf, "ulAPRtn=0x%08X\n", ulAPRtn);
//    printf(debugbuf);
    if (ulAPRtn == 0xA0000001) {
		/*
        switch (stRCData.bSID) {
            case 0x20://0x01:
                sprintf(sBuf, "%s", "PayPass M-Stripe");
                ShowReverseLine(1, 1, sBuf);
                break;
            case 0x21://0x02:
                sprintf(sBuf, "%s", "PayPass MChip");
                ShowReverseLine(1, 1, sBuf);
                break;
            case 0x13://0x09:
                sprintf(sBuf, "%s", "VISA Old US MSD");
                ShowReverseLine(1, 1, sBuf);
                break;
            case 0x18://0x13:
                sprintf(sBuf, "%s", "VisaWave MSD");
                ShowReverseLine(1, 1, sBuf);
                break;
            case 0x16:
                sprintf(sBuf, "%s", "VisaWave 2");
                ShowReverseLine(1, 1, sBuf);
                break;
            case 0x17:
                sprintf(sBuf, "%s", "VisaWave qVSDC");
                ShowReverseLine(1, 1, sBuf);
                break;
            case 0x40:
                sprintf(sBuf, "%s", "American Express");
                ShowReverseLine(1, 1, sBuf);
                break;
            case 0x61:
                sprintf(sBuf, "%s", "J/Speedy");
                ShowReverseLine(1, 1, sBuf);
                break;
            default:
                sprintf(sBuf, "%s", "CardType No Def.");
                ShowReverseLine(1, 1, sBuf);
                break;
        }
		*/
...
void Polling_Card(void)
{
	...
	//EMVC
	CTOS_KMS2Init();
	usRtn = EMV_Initialize(&g_emv_event, d_EMV_CONFIG_FILE); 
    if(usRtn != d_OK)
    {
        sprintf(LCDbuff, "initial ret %04X ", usRtn);
...
int main(int argc, char *argv[]) {
    ...
    if (ulAPRtn != d_NO_ERROR) {
        ...
        while (1) {
			...
			CTOS_LCDGShowBMPPic(0, 0, "Welcome.bmp");
			CTOS_LCDTPrintXY(1, 5, " Enter Sale Amount: ");
			CTOS_LCDTPrintXY(1, 6, "   $          0.00  ");
			ulAPRtn = InputValue();
			if (ulAPRtn == FALSE) {
				break;
            ...
            //StartTrans();
            CTOS_Delay(1000);
            //CTOS_LCDTPrintXY(1, 7, "Power Key-->Exit");
            //CTOS_LCDTPrintXY(1, 8, "Others-->Continue");
            bKey = d_KBD_ENTER;
            if (bKey == d_KBD_CLEAR) {
                CTOS_LCDTClearDisplay();
